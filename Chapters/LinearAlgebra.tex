...
Vectors in quantum mechanic are denoted by \(\ket{\psi}\). Its conjugate transpose is \(\bra{\psi} = \ket{\psi}^{\dagger}\) and we can define a inner product \(\angleBracket{\phi,\psi} = \braket{\phi}{\psi}\). Given an inner product we can define norm 
\begin{equation*}
    \norm{\ket{\psi}} = \sqrt{\braket{\psi}{\psi}}
\end{equation*}
The Pauli matrices are \(\squareMatrices[\Complex]{2}\) 
\begin{align*}
...
For \(\ket{v} \in V, \ket{w} \in W\) we can define an outer product \(\ket{w}\bra{v} : V \to W\). 
\begin{equation*}
    \ket{w} \bra{v} \ket{v'} = \braket{v}{v'} \ket{w}
\end{equation*}
The completeness relation say that given an orthogonal basis \(\ket{i}\) 
\begin{equation*}
    \sum \ket{i} \bra{i} = I 
\end{equation*}
which is easy to see 
\begin{equation*}
    \sum \ket{i} \bra{i} \ket{v} = \sum \braket{i}{v} \ket{i} = v
\end{equation*}
Given an inner product vector space we can define the Cauchy-Schwarz inequality 
\begin{equation*}
    \braket{v}{v} \braket{w}{w} \geq \abs{\braket{v}{w}}^2
\end{equation*}

\section{Adjoint and Hermitian operators}
For any linear operator on a Hilbert space there exists \(B\) such that for all \(v,w\in V\) 
\begin{equation*}
    \braket{v}{Aw} = \braket{Bv}{w}
\end{equation*}
It is easy to see that \(B = A^{\dagger}\) the adjoint or Hermitian conjuage of \(A\). \(A = A^{\dagger}\) is a Hermitian or self-adjoint operator. A projection is an operator that projects \(\ket{v}\) into its compenents on a subspace. Suppose \(\ket{1} , \dots , \ket{k}\) is an orthonormal basis for subspace \(W\) 
\begin{equation*}
